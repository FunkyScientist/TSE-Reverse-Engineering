...
class TransformUtils {
    ...
    public static FrameImage2D makeTempFrame(FrameImage2D frameImage2D, int[] iArr) {
        if (frameImage2D == null) {
            return Frame.create(FrameType.image2D(FrameType.ELEMENT_RGBA8888, 18), iArr).asFrameImage2D();
        }
        if (!Arrays.equals(iArr, frameImage2D.getDimensions())) {
            frameImage2D.resize(iArr);
            return frameImage2D;
        }
        return frameImage2D;
    }
    ...
    public static void setTextureParameter(FrameImage2D frameImage2D, int i, int i2) {
